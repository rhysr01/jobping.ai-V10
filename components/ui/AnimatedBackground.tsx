...
export default function AnimatedBackground() {
	...
	const orb1X = useTransform(
		[mouseX, scrollY],
		([mx, sy]: number[]) => mx * 0.2 + (sy / 1000) * 150,
	);
	const orb1Y = useTransform(
		[mouseY, parallaxSlow],
		([my, ps]: number[]) => my * 0.2 + ps,
	);
	...
	const orb2X = useTransform(
		[mouseX, scrollY],
		([mx, sy]: number[]) => -(mx * 0.15) - (sy / 1000) * 120,
	);
	const orb2Y = useTransform(
		[mouseY, parallaxMedium],
		([my, pm]: number[]) => -(my * 0.15) + pm,
	);
	...
	const orb3X = useTransform(
		[mouseX, scrollY],
		([mx, sy]: number[]) => mx * 0.1 + (sy / 1000) * 90,
	);
	const orb3Y = useTransform(
		[mouseY, parallaxFast],
		([my, pf]: number[]) => my * 0.1 + pf,
	);
	const orb3Scale = useTransform(
		scrollY,
		[0, 1000],
		[1, isMobile ? 1.2 : 1.25],
	);
