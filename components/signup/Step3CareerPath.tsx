			...
			<div className="relative overflow-hidden rounded-3xl border border-brand-500/20 bg-gradient-to-br from-zinc-900/80 via-zinc-900/60 to-zinc-800/80 backdrop-blur-sm px-4 py-5 sm:px-6 sm:py-6 md:px-8 md:py-8">
				...
				<div className="relative z-10 space-y-6 sm:space-y-8 md:space-y-10 lg:space-y-12">
					<div className="mb-6 sm:mb-8">
						...
						>
							Discover the perfect career direction for your skills and
							interests
						</motion.p>
						...
						>
							...
							<div className="space-y-3">
								...
								<motion.div
									...
									animate={
										formData.roles.length > 0 ? { scale: [1, 1.02, 1] } : {}
									}
					...
					<div>
						...
						<p className="text-sm text-zinc-400 mb-6">
							...
							{maxSelections > 1
								...
								: "the career path"}{" "}
							that interest{maxSelections > 1 ? "" : "s"} you most
						</p>
						<div
										...
										onClick={() => {
											...
											if (newCareer) {
												...
												if (isSelected) {
													...
													newCareerPaths = formData.careerPath.filter(
														(cp) => cp !== path.value,
													);
												} else {
												...
												const validRoles = formData.roles.filter((role) => {
													return newCareerPaths.some((cp) => {
														const career = CAREER_PATHS.find(
															(c) => c.value === cp,
														);
					...
					{(() => {
						const selectedCareers = CAREER_PATHS.filter((c) =>
							formData.careerPath.includes(c.value),
						);
						...
						return (
							...
							>
								...
								<div className="flex flex-col sm:flex-row gap-2 mb-4">
									...
									>
										Select Popular Roles (
										{primaryCareer.popularRoles?.length ||
											primaryCareer.roles.length}
										)
									</motion.button>
									<motion.button
										type="button"
