...
interface Step1BasicsFixedProps {
  onNext: (data: { fullName: string; email: string; university?: string }) => void;
  initialData?: { fullName?: string; email?: string; university?: string };
}

export function Step1BasicsFixed({ onNext, initialData = {} }: Step1BasicsFixedProps) {
  const form = useForm({
  ...
  const onSubmit = (data: { fullName: string; email: string; university?: string }) => {
