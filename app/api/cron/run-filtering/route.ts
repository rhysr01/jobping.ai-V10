...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      let govCount = 0;
      const govPatterns = ['politician', 'government', 'minister', 'ambassador', 'diplomat', 'parliament', 'council', 'civil servant', 'public sector', 'policy', 'legislation'];
      ...
      for (const pattern of govPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%government_political_role%')
          .select('id');
      ...
      results.push({ step: "Government & Political Filtering", count: govCount, status: "success" });
      console.log(`✅ Filtered ${govCount} government/political roles`);
    } catch (error: any) {
      results.push({ step: "Government & Political Filtering", status: "failed", error: error.message });
    ...
    try {
      let militaryCount = 0;
      const militaryPatterns = ['military', 'armed forces', 'navy', 'army', 'air force', 'defense', 'security guard', 'security officer'];
      ...
      for (const pattern of militaryPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%military_defense_role%')
          .select('id');
      ...
      results.push({ step: "Military & Defense Filtering", count: militaryCount, status: "success" });
      console.log(`✅ Filtered ${militaryCount} military/defense roles`);
    } catch (error: any) {
      results.push({ step: "Military & Defense Filtering", status: "failed", error: error.message });
    ...
    try {
      ...
      const entertainmentPatterns = ['athlete', 'actor', 'actress', 'musician', 'singer', 'performer', 'entertainment', 'fitness trainer', 'gym instructor', 'personal trainer'];
      ...
      for (const pattern of entertainmentPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%entertainment_sports_role%')
          .select('id');
      ...
      results.push({ step: "Entertainment & Sports Filtering", count: entertainmentCount, status: "success" });
      console.log(`✅ Filtered ${entertainmentCount} entertainment/sports roles`);
    } catch (error: any) {
      results.push({ step: "Entertainment & Sports Filtering", status: "failed", error: error.message });
    ...
    try {
      ...
      const hospitalityPatterns = ['waiter', 'waitress', 'bartender', 'barista', 'hotel', 'receptionist', 'housekeeper', 'tour guide', 'tourism', 'restaurant'];
      ...
      for (const pattern of hospitalityPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%hospitality_service_role%')
          .select('id');
      ...
      results.push({ step: "Hospitality & Service Filtering", count: hospitalityCount, status: "success" });
      console.log(`✅ Filtered ${hospitalityCount} hospitality/service roles`);
    } catch (error: any) {
      results.push({ step: "Hospitality & Service Filtering", status: "failed", error: error.message });
    ...
    try {
      let retailCount = 0;
      const retailPatterns = ['cashier', 'sales assistant', 'shop assistant', 'retail assistant', 'store assistant', 'checkout'];
      ...
      for (const pattern of retailPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%retail_sales_role%')
          .select('id');
      ...
      results.push({ step: "Retail & Sales Filtering", count: retailCount, status: "success" });
      console.log(`✅ Filtered ${retailCount} retail/sales roles`);
    } catch (error: any) {
      results.push({ step: "Retail & Sales Filtering", status: "failed", error: error.message });
    ...
    try {
      ...
      const manualPatterns = ['mechanic', 'electrician', 'plumber', 'carpenter', 'welder', 'painter', 'driver', 'delivery driver', 'taxi driver', 'truck driver'];
      ...
      for (const pattern of manualPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%manual_labor_trade_role%')
          .select('id');
      ...
      results.push({ step: "Manual Labor & Trades Filtering", count: manualLaborCount, status: "success" });
      console.log(`✅ Filtered ${manualLaborCount} manual labor/trade roles`);
    } catch (error: any) {
      results.push({ step: "Manual Labor & Trades Filtering", status: "failed", error: error.message });
    ...
    try {
      ...
      const realEstatePatterns = ['real estate agent', 'property agent', 'insurance agent', 'insurance broker', 'loan officer', 'mortgage', 'financial advisor'];
      ...
      for (const pattern of realEstatePatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%real_estate_insurance_role%')
          .select('id');
      ...
      results.push({ step: "Real Estate & Insurance Filtering", count: realEstateCount, status: "success" });
      console.log(`✅ Filtered ${realEstateCount} real estate/insurance roles`);
    } catch (error: any) {
      results.push({ step: "Real Estate & Insurance Filtering", status: "failed", error: error.message });
    ...
    try {
      ...
      const callCenterPatterns = ['telemarketer', 'telemarketing', 'call center', 'call centre', 'customer service rep', 'phone operator'];
      ...
      for (const pattern of callCenterPatterns) {
        const { data, error } = await supabase
          ...
          .ilike('title', `%${pattern}%`)
          .eq('is_active', true)
          .or('filtered_reason.is.null,filtered_reason.not.like.%call_center_telemarketing_role%')
          .select('id');
      ...
      results.push({ step: "Call Center & Telemarketing Filtering", count: callCenterCount, status: "success" });
      console.log(`✅ Filtered ${callCenterCount} call center/telemarketing roles`);
    } catch (error: any) {
      results.push({ step: "Call Center & Telemarketing Filtering", status: "failed", error: error.message });
    ...
    try {
      let ceoCount = 0;
      const ceoPatterns = ['ceo', 'chief executive', 'managing director', 'cfo', 'cto', 'coo', 'cmo'];
    ...
    try {
      ...
      const { data: suspiciousJobs, error: suspiciousError } = await supabase
        ...
        .or('title.ilike.%test%,title.ilike.%fake%,title.ilike.%dummy%,company.ilike.%test%')
        .eq('is_active', true)
        .select('id');
      ...
      results.push({ step: "Metadata Quality Filtering", count: metadataCount, status: "success" });
      console.log(`✅ Filtered ${metadataCount} metadata quality issues`);
    } catch (error: any) {
      results.push({ step: "Metadata Quality Filtering", status: "failed", error: error.message });
