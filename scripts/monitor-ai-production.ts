...
class AIProductionMonitor {
	...
	constructor() {
		if (
			...
			throw new Error(
				"Supabase credentials required for production monitoring",
			);
		}
	...
	async generateProductionReport(
		...
		console.log(
			`📊 Generating AI Production Report (last ${hoursBack} hours)\n`,
		);
	...
	private async getMatchMetrics(_startTime: Date, _endTime: Date) {
	...
	private async assessQualityMetrics(_startTime: Date, _endTime: Date) {
	...
	private async getPerformanceMetrics(_startTime: Date, _endTime: Date) {
	...
	private calculateQualityScore(
		...
		const matchScore =
			matchMetrics.averageMatches >= 4.5
				? 100
				: matchMetrics.averageMatches >= 3.5
					? 80
					: 60;

		const qualityScore =
			qualityMetrics.locationAccuracy * 0.4 +
			qualityMetrics.titleSpecificity * 0.4 +
			(100 - qualityMetrics.falsePositiveRate) * 0.2;

		const performanceScore =
			performanceMetrics.averageResponseTime <= 3
				? 100
				: performanceMetrics.averageResponseTime <= 5
					? 80
					: performanceMetrics.averageResponseTime <= 10
						? 60
						: 40;

		const diversityScore =
			qualityMetrics.companyDiversity >= 4
				? 100
				: qualityMetrics.companyDiversity >= 3
					? 80
					: qualityMetrics.companyDiversity >= 2
						? 60
						: 40;
	...
	private generateAlerts(
		...
		if (matchMetrics.averageMatches < 3) {
			alerts.push(
				"🚨 CRITICAL: Average matches too low (< 3) - AI matching broken",
			);
		}

		if (qualityMetrics.locationAccuracy < 80) {
			alerts.push(
				"🚨 CRITICAL: Location accuracy too low (< 80%) - Geographic targeting failed",
			);
		}

		if (performanceMetrics.averageResponseTime > 15) {
			alerts.push(
				"🚨 CRITICAL: Response time too slow (> 15s) - Performance degradation",
			);
		}
		...
		if (qualityMetrics.falsePositiveRate > 40) {
			alerts.push(
				"⚠️ WARNING: High false positive rate (> 40%) - too many irrelevant matches",
			);
		}
	...
	private displayReport(report: ProductionMetrics): void {
		...
		console.log("-".repeat(30));
		console.log(
			`⏱️  Average Response: ${report.performanceMetrics.averageResponseTime}s`,
		);
		console.log(
			`📍 95th Percentile: ${report.performanceMetrics.p95ResponseTime}s`,
		);
		...
		console.log("-".repeat(30));
		console.log(
			`📍 Location Accuracy: ${report.qualityMetrics.locationAccuracy}%`,
		);
		console.log(
			`🏢 Company Diversity: ${report.qualityMetrics.companyDiversity}/5`,
		);
		console.log(
			`📋 Title Specificity: ${report.qualityMetrics.titleSpecificity}%`,
		);
		console.log(
			`🚫 False Positive Rate: ${report.qualityMetrics.falsePositiveRate}%`,
		);
		console.log("");
...
async function main() {
	try {
		...
		const hoursBack = process.argv[2] ? parseInt(process.argv[2], 10) : 24;

		console.log(
			`🔍 Analyzing AI performance in production (last ${hoursBack} hours)...\n`,
		);
		...
		const exitCode =
			report.qualityScore >= 75 ? 0 : report.qualityScore >= 60 ? 1 : 2;
