...
function categorizeJob(title, description = '', currentCategories = []) {
  ...
  const hasExperienceCategory = ['early-career', 'internship', 'graduate'].some(cat => newCategories.has(cat));
  const hasCareerCategory = Object.keys(CAREER_PATH_MAPPINGS).some(cat => newCategories.has(cat));
...
async function fixJobCategorization() {
  ...
  try {
    ...
    for (const job of jobs) {
      const newCategories = categorizeJob(job.title, job.description, job.categories);

      // Check if categories changed
      const currentCats = new Set(job.categories || []);
      const newCats = new Set(newCategories);
      const categoriesChanged = currentCats.size !== newCats.size ||
...
async function fixLocationData() {
  ...
  try {
    ...
    if (nullError) {
      ...
      console.log(`📍 Jobs with missing location data: ${nullLocations.length}`);
    }
