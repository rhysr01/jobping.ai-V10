...
class TestHealthDashboard {
	...
	private displaySummary(): void {
		...
		const { totalTests, passedTests, failedTests, passRate, duration } =
			this.metrics;
		...
		console.log(
			`📅 Last Updated: ${new Date(this.metrics.timestamp).toLocaleString()}`,
		);
		console.log();
	}
	...
	private displayCategoryBreakdown(): void {
		...
		Object.entries(categories).forEach(([key, data]) => {
			...
			const passRate =
				total > 0 ? ((data.passed / total) * 100).toFixed(1) : "0.0";
			const status = data.failed === 0 ? "✅" : data.failed <= 2 ? "⚠️ " : "❌";

			console.log(
				`${status} ${categoryNames[key as keyof typeof categoryNames]}`,
			);
			console.log(
				`   ${data.passed}/${total} passed (${passRate}%) - ${this.formatDuration(data.duration)}`,
			);
		});
		console.log();
	}
	...
	private displayPerformanceInsights(): void {
		...
		console.log(
			`🐌 Slowest Test: ${performance.slowestTest.name} (${performance.slowestTest.duration}s)`,
		);
		console.log(
			`🚀 Fastest Test: ${performance.fastestTest.name} (${performance.fastestTest.duration}s)`,
		);
		...
		benchmarks.forEach((benchmark) => {
			console.log(
				`   ${benchmark.status} ${benchmark.label}: ${benchmark.actual}s (${benchmark.target})`,
			);
		});
		console.log();
	}
	...
	private generateRecommendations(): string[] {
		...
		if (reliability.passRate < 95) {
			recommendations.push(
				"🔧 Investigate root causes of test failures - consider environmental factors",
			);
		}

		if (reliability.flakyTests.length > 0) {
			recommendations.push(
				"🎯 Address flaky tests with better isolation and retry logic",
			);
		}

		// Performance recommendations
		if (this.metrics.duration > 900) {
			// 15 minutes
			recommendations.push(
				"⚡ Optimize test execution time - consider parallelization or selective running",
			);
		}
		...
		if (testCategories.visual.failed > 0) {
			recommendations.push(
				"👁️ Review visual regression failures - update baselines if changes are intentional",
			);
		}

		if (testCategories.chaos.failed > 0) {
			recommendations.push(
				"🧪 Investigate chaos engineering failures - ensure graceful degradation works",
			);
		}
		...
		if (this.metrics.coverage && this.metrics.coverage.lines < 85) {
			recommendations.push(
				"📊 Improve test coverage - focus on critical user paths",
			);
		}
		...
		if (reliability.passRate >= 95) {
			recommendations.push(
				"🎉 Excellent test reliability! Continue monitoring and maintaining quality standards",
			);
		}
