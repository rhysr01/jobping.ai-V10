...
async function testMatchingCoordinator() {
	...
	try {
		...
		const {
			coordinatePremiumMatching,
		} = require("./Utils/matching/guaranteed/coordinator.ts");

		const result = await coordinatePremiumMatching(jobs, userPrefs, supabase, {
			targetCount: 10,
		...
		if (result.matches.length > 0) {
			...
			result.matches.slice(0, 3).forEach((match, i) => {
				console.log(
					`  ${i + 1}. ${match.job.title} at ${match.job.company} (${match.match_score}%)`,
				);
			});
		}
	} catch (error) {
