...
class MigrationOrganizer {
		...
		const files = readdirSync(this.migrationDir)
			...
			.map((filename) => {
				...
				if (
					...
					content.includes("UPDATE jobs") ||
					content.includes("filtered_reason") ||
	...
	generateReport(files: MigrationFile[]): void {
		...
		if (duplicates.length > 0) {
			issues.push(
				`Duplicate timestamps: ${[...new Set(duplicates)].join(", ")}`,
			);
		}
		...
		if (largeFiles.length > 0) {
			issues.push(
				`${largeFiles.length} files larger than 50KB (may be too complex)`,
			);
		}
	...
	organizeFiles(files: MigrationFile[]): void {
		...
		files.forEach((file) => {
			...
			try {
				...
				console.error(
					`   ❌ Failed to organize ${file.filename}: ${error.message}`,
				);
			}
		});
...
${files
	.slice(-10)
	.map((f) => `- ${f.timestamp}: ${f.description} (${f.category})`)
	.join("\\n")}
...
`;
		...
		console.log(
			"   ✅ Generated cleanup script: scripts/cleanup-migrations.sh",
		);
	}
	...
	run(): void {
		...
		console.log("");
		console.log(
			"📂 Organized files available in: supabase/migrations_organized/",
		);
