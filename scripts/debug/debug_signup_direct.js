...
async function debugSignupIssues() {
	...
	try {
		...
		const { data: recentPremiumUsers, error: premiumUsersError } =
			await supabase
				.from("users")
				.select(
					"email, full_name, created_at, email_count, last_email_sent, subscription_tier",
				)
				.eq("subscription_tier", "premium")
		...
		if (premiumUsersError) {
			...
			console.log(
				`   Found ${recentPremiumUsers.length} recent premium users:`,
			);
			for (const user of recentPremiumUsers) {
				...
				if (matchesError) {
					...
					matches.forEach((match, i) => {
						console.log(
							`     ${i + 1}. Score: ${(match.match_score * 100).toFixed(1)}% - ${match.match_reason}`,
						);
					});
				}
			}
		...
		if (eventsError) {
			...
			signupEvents.forEach((event) => {
				console.log(
					`   - ${event.created_at}: ${JSON.stringify(event.properties || {})}`,
				);
			});
		}
	} catch (error) {
