...
class AdvancedMigrationRunner {
	...
	constructor() {
		...
		const projectRef = this.supabaseUrl
			.replace("https://", "")
			.replace(".supabase.co", "");
	...
	private organizeMigrationsIntoBatches(
		migrations: Migration[],
	): MigrationBatch[] {
		...
		const securityMigrations = migrations.filter(
		...
		const dataMigrations = migrations.filter(
			(m) => m.category === "data" && !processed.has(m.id),
		);
		...
		const maintenanceMigrations = migrations.filter(
			(m) => m.category === "maintenance" && !processed.has(m.id),
		);
	...
	private async runBatch(batch: MigrationBatch): Promise<void> {
		...
		if (batch.canRunParallel && batch.migrations.length > 1) {
			...
			const promises = batch.migrations.map((migration) =>
			...
			for (const migration of batch.migrations) {
				...
				if (result.status === "failed" && batch.category === "security") {
					console.error(
						`   🚨 Critical failure in ${batch.category} - stopping batch`,
					);
					break;
				}
			}
	...
	async runMigrations(): Promise<void> {
		...
		batches.forEach((batch) => {
			console.log(
				`   ${batch.category}: ${batch.migrations.length} migrations (${batch.canRunParallel ? "parallel" : "sequential"})`,
			);
		});
		console.log("");
