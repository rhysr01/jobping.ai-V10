...
class DeploymentMonitor {
  ...
  private async monitorDeployment(deploymentUrl: string): Promise<void> {
    ...
    console.log("🔍 You can check the deployment status manually in Vercel dashboard");
    process.exit(1);
  }

  private async getDeploymentLogs(deploymentUrl: string): Promise<void> {
    try {
  ...
  private async notifyFailure(deploymentUrl: string, status: string): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      case "--timeout":
        ...
        break;
      case "--interval":
        options.pollInterval = parseInt(args[++i]) * 1000;
        break;
