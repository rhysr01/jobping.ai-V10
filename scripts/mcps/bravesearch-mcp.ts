export class BraveSearchMCP {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn("⚠️  BraveSearch MCP: BRAVE_API_KEY not set. BraveSearch tools will not be available.");
    }
  }
  ...
  async webSearch(args: any) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`BraveSearch API error: ${response.status} ${response.statusText}`);
      }
  ...
  async researchTopic(args: any) {
    ...
    try {
      ...
      const mainSearch = await this.webSearch({ query: topic, count, safesearch: "strict" });

      if (include_stats) {
        // Search for related statistics/data
        const statsQuery = `${topic} statistics 2024`;
        const statsSearch = await this.webSearch({ query: statsQuery, count: 5, safesearch: "strict" });
  ...
  async findSolutions(args: any) {
    ...
    try {
      ...
      const searchResults = await this.webSearch({ query, count, safesearch: "moderate" });
  ...
  async techDocumentation(args: any) {
    ...
    try {
      ...
      const searchResults = await this.webSearch({ query, count, safesearch: "strict" });
