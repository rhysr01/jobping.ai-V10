...
export class PlaywrightMCP {
  constructor() {
    // No shared browser - create fresh for each operation
  }
  ...
  private async createContext(browser: any, viewport = { width: 1280, height: 720 }) {
    ...
    try {
      ...
      for (const action of actions) {
        try {
          switch (action.type) {
            ...
            case 'assert':
              ...
              if (action.expected === 'visible' && isVisible) {
                results.push(`✅ Assertion passed: ${action.selector} is visible`);
              } else if (action.expected === 'hidden' && !isVisible) {
                results.push(`✅ Assertion passed: ${action.selector} is hidden`);
              } else {
                results.push(`❌ Assertion failed: ${action.selector} visibility expected ${action.expected}, got ${isVisible ? 'visible' : 'hidden'}`);
              }
              break;
          ...
          results.push(`❌ Action failed (${action.type}): ${actionError.message}`);
        }
      }
