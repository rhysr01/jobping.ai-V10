...
export class PuppeteerMCP {
  ...
  async takeScreenshot(args: any) {
    const { url, viewport = { width: 1280, height: 720 }, fullPage = false, selector } = args;
  ...
  async analyzePageDesign(args: any) {
    ...
    try {
      ...
      const designAnalysis = await page.evaluate(() => {
        ...
        analysis.headings = Array.from(headings).map(h => `${h.tagName}: ${h.textContent?.trim()}`);
        ...
        // Layout analysis
        analysis.layout.hasHero = !!document.querySelector('[class*="hero"], [id*="hero"], header, .banner');
        analysis.layout.hasNavigation = !!document.querySelector('nav, [class*="nav"], [role="navigation"]');
        analysis.layout.hasFooter = !!document.querySelector('footer, [class*="footer"]');
        ...
        analysis.performance.imagesWithoutAlt = Array.from(images).filter(img => !img.alt).length;

        const allElements = document.querySelectorAll('*');
        analysis.performance.missingTitles = Array.from(allElements).filter(el => el.hasAttribute('title') && !el.getAttribute('title')).length;

        // Accessibility issues
        analysis.accessibility.missingAltText = analysis.performance.imagesWithoutAlt;
      ...
      const report = this.generateDesignReport(designAnalysis, url, compareWith);
  ...
  async comparePages(args: any) {
    ...
    try {
      ...
      const comparison = this.generateComparisonReport(analysis1, analysis2, url1, url2, aspect);
  ...
  private async analyzeSinglePage(url: string) {
    ...
    const analysis = await page.evaluate(() => ({
      ...
      buttonsCount: document.querySelectorAll('button, [role="button"], a[class*="btn"]').length,
      formFieldsCount: document.querySelectorAll('input, textarea, select').length,
      colorScheme: Array.from(new Set(
      ...
      hasCTA: !!document.querySelector('[class*="cta"], [class*="call-to-action"], button, a[class*="btn"]'),
  ...
  private generateDesignReport(analysis: any, url: string, compareWith?: string): string {
    ...
    if (!analysis.layout.responsive) recommendations.push("• Add responsive design with viewport meta tag");
    if (analysis.performance.imagesWithoutAlt > 0) recommendations.push("• Add alt text to all images for accessibility");
    if (analysis.accessibility.smallText > 0) recommendations.push("• Increase font sizes for better readability");
    if (!analysis.layout.hasHero) recommendations.push("• Consider adding a hero section for better first impression");
  ...
  private generateComparisonReport(analysis1: any, analysis2: any, url1: string, url2: string, aspect: string): string {
    let report = `# 🔍 Page Comparison: ${aspect.toUpperCase()}\n\n`;
    report += `**${url1}** vs **${url2}**\n\n`;

    const comparisons = [
      { label: "Page Title", val1: analysis1.title, val2: analysis2.title },
      { label: "Headings Count", val1: analysis1.headingsCount, val2: analysis2.headingsCount },
      { label: "Images Count", val1: analysis1.imagesCount, val2: analysis2.imagesCount },
      { label: "Buttons/CTAs", val1: analysis1.buttonsCount, val2: analysis2.buttonsCount },
      { label: "Form Fields", val1: analysis1.formFieldsCount, val2: analysis2.formFieldsCount },
      { label: "Hero Section", val1: analysis1.hasHero ? "✅" : "❌", val2: analysis2.hasHero ? "✅" : "❌" },
      { label: "Call-to-Action", val1: analysis1.hasCTA ? "✅" : "❌", val2: analysis2.hasCTA ? "✅" : "❌" },
      { label: "Mobile Friendly", val1: analysis1.mobileFriendly ? "✅" : "❌", val2: analysis2.mobileFriendly ? "✅" : "❌" },
    ];
    ...
    report += `## 💡 Insights\n`;
    const insights = this.generateComparisonInsights(analysis1, analysis2, url1, url2);
    report += insights;
  ...
  private generateComparisonInsights(analysis1: any, analysis2: any, url1: string, url2: string): string {
    const insights = [];

    if (analysis1.headingsCount > analysis2.headingsCount) {
      insights.push(`• ${url1} has better content structure with more headings`);
