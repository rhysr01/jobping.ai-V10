...
export class SupabaseMCP {
	...
	constructor() {
		...
		if (!supabaseUrl || !supabaseKey) {
			console.warn(
				"⚠️  Supabase MCP: Missing environment variables. Supabase tools will not be available.",
			);
			console.warn(
	...
	async queryUsers(args: any) {
		...
		try {
			...
			let query = this.supabase
				.from("users")
				.select(
					"id, email, created_at, updated_at, user_type, subscription_status",
				)
	...
	async queryJobs(args: any) {
		...
		try {
			...
			let query = this.supabase
				.from("jobs")
				.select(
					"id, title, company, location, job_type, created_at, updated_at, status",
				)
				.range(offset, offset + limit - 1)
				.order("created_at", { ascending: false });
				...
				try {
					...
					if (error) {
						...
						if (recentRecords && recentRecords.length > 0) {
							stats[table].latest_record = new Date(
								recentRecords[0].created_at,
							).toLocaleString();
						}
					}
				} catch (error: any) {
