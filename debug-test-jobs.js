	...
	try {
		...
		const techJobs = jobs.filter(
			(job) =>
				...
				(job.categories.includes("tech-transformation") ||
		...
		// Check London jobs
		const londonJobs = jobs.filter(
			(job) => job.city && job.city.toLowerCase().includes("london"),
		);
		...
		if (londonJobs.length > 0) {
			londonJobs.forEach((job) => {
				console.log(
					`   ${job.title} @ ${job.company} - ${job.city} - ${JSON.stringify(job.categories)}`,
				);
			});
		}
	} catch (error) {
