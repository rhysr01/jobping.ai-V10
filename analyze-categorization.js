...
async function analyzeCategories() {
	...
	console.log(
		`   Internships: ${internships} (${((internships / totalActive) * 100).toFixed(1)}%)`,
	);
	console.log(
		`   Graduate Roles: ${graduates} (${((graduates / totalActive) * 100).toFixed(1)}%)`,
	);
	console.log(
		`   Early Career: ${earlyCareer} (${((earlyCareer / totalActive) * 100).toFixed(1)}%)`,
	);
	console.log(`   Sum of categories: ${internships + graduates + earlyCareer}`);
	console.log(
		`   Uncategorized: ${totalActive - (internships + graduates + earlyCareer)}\n`,
	);
	...
	if (overlap && overlap.length > 0) {
		console.log(
			"⚠️  OVERLAP DETECTED - Jobs marked as BOTH internship AND graduate:",
		);
		overlap.forEach((job) => {
	...
	internshipCheck?.forEach((row) => {
		console.log(
			`        ${row.is_internship}       |      ${row.is_graduate}       | ${row.count}`,
		);
	});
}
