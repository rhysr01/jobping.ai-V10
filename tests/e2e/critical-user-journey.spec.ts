...
test.describe("Critical User Journey - Free Signup to Matches", () => {
	...
	test("complete free user journey: signup → matches → email", async ({
		page,
	}) => {
		...
		const cityButton = page
			.locator('[role="button"]')
			.filter({ hasText: /London|Berlin|Paris/ })
			.first();
		await expect(cityButton).toBeVisible({ timeout: 5000 });
		await cityButton.click();

		// Select career path
		const careerCheckbox = page
		...
		const ageCheckbox = page
			.locator('input[type="checkbox"]')
			.filter({ hasText: /16|age/i })
			.first();
		await ageCheckbox.check();

		const termsCheckbox = page
			.locator('input[type="checkbox"]')
			.filter({ hasText: /terms|accept/i })
			.first();
		...
		const submitButton = page
			...
			.filter({ hasText: /submit|signup|register|get matches/i })
			.first();
		...
		const matchCards = page
			.locator('[data-testid="job-card"], .job-card, .match-card')
			.first();
		...
		await expect(page.locator("text=Welcome|Check your email")).toBeVisible({
			timeout: 5000,
		});
	...
	test("signup form validation works", async ({ page }) => {
		...
		const submitButton = page
			.locator('button[type="submit"], button')
			.filter({ hasText: /submit|signup|get matches/i })
			.first();
		await submitButton.click();

		// Should show validation errors
		await expect(page.locator("text=required|invalid|please fill")).toBeVisible(
			{ timeout: 5000 },
		);
	...
	test("homepage loads and shows key elements", async ({ page }) => {
		...
		await expect(
			page.locator("text=JobPing|Find your dream job"),
		).toBeVisible();
		await expect(
			page
				.locator("button, a")
				.filter({ hasText: /Get Started|Sign Up|Start/i }),
		).toBeVisible();
