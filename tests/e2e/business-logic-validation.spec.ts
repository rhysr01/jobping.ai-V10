...
test.describe("Business Logic Validation", () => {
    ...
    test("UserChoiceRespector: City Distribution Works", async ({ page }) => {
        ...
        const matchLocations = await page.locator('[data-testid="job-location"], .job-location').allTextContents();
        const uniqueCities = [...new Set(matchLocations.map(loc =>
            cities.find(city => loc.toLowerCase().includes(city.toLowerCase()))
        ).filter(Boolean))];
    ...
    test("UserChoiceRespector: Source Diversity Works", async ({ page }) => {
        ...
        const companyNames = await page.locator('[data-testid="job-company"], .job-company').allTextContents();
    ...
    test("Premium User: Career Path Balance (if 2 paths selected)", async ({ page }) => {
        ...
        console.log("Premium career path balance test - requires full premium flow implementation");
    });
});