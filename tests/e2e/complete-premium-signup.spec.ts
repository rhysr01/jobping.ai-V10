...
test.describe("Complete Premium Signup Flow", () => {
	...
	test("README Flow 1: 4-Step Premium Form → Email Verification", async ({
		page,
	}) => {
		...
		await page
			.locator("button")
			.filter({ hasText: testData.experience })
			.first()
			.click();

		await page
			.locator('input[type="radio"]')
			.filter({ hasText: testData.visaStatus })
			.first()
			.check();
		await page
			.locator("button")
		...
		await expect(
			page.locator("text=Email Verification Required"),
		).toBeVisible();
	...
	test("README Flow 2: Email Verification → Payment Flow", async ({ page }) => {
		...
		await page.goto(
			`/signup/verify?email=${encodeURIComponent(testData.email)}`,
		);
