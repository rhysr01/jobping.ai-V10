...
test.describe("Real User Scenarios - Premium Users", () => {
	test("Premium User Gets 15 Enhanced Matches vs Free User's 5", async ({
		request,
	}) => {
		...
		if (freeMatches.status() === 200 && premiumMatches.status() === 200) {
			...
			const premiumAvgScore =
				premiumData.jobs.reduce(
					(sum: number, job: any) => sum + job.unifiedScore.overall,
					0,
				) / premiumData.jobs.length;

			const freeAvgScore =
				freeData.jobs.reduce(
					(sum: number, job: any) => sum + job.unifiedScore.overall,
					0,
			...
			expect(premiumData.targetCompanies.length).toBeGreaterThan(
				freeData.targetCompanies.length,
			);
		}
	});
	...
	test("Premium User Gets Enhanced Visa Information", async ({ request }) => {
		...
		if (matchesResponse.status() === 200) {
			...
			expect(matchesData.premiumFeatures).toHaveProperty(
	...
	test("Premium User Gets Balanced Career Path Distribution", async ({
		request,
	}) => {
	...
	test("Premium User Gets Higher Quality Companies", async ({ request }) => {
		...
		if (freeMatches.status() === 200 && premiumMatches.status() === 200) {
			...
			const premiumHasMoreData = premiumData.jobs.filter(
				(job: any) =>
			...
			const freeHasMoreData = freeData.jobs.filter(
				(job: any) =>
					job.company_profile_url ||
					job.work_arrangement ||
					job.employment_type,
	...
	test("Premium User Upgrades from Free Gets Better Matches", async ({
		request,
	}) => {
		...
		if (premiumMatches.status() === 200) {
			...
			// 🎯 REAL USER VALIDATION: Premium features enabled
			expect(premiumData.premiumFeatures).toHaveProperty(
				"enhancedVisaConfidence",
				true,
			);
			expect(premiumData.premiumFeatures).toHaveProperty(
			...
			const avgPremiumScore =
				premiumData.jobs.reduce(
					(sum: number, job: any) => sum + (job.unifiedScore?.overall || 0),
					0,
				) / premiumData.jobs.length;

			const premiumHasEnhancedData = premiumData.jobs.filter(
				(job: any) =>
					job.work_arrangement || job.employment_type || job.visa_confidence,
			).length;

			console.log(`Average premium job score: ${avgPremiumScore.toFixed(2)}`);
			console.log(
				`Jobs with enhanced data: ${premiumHasEnhancedData}/15 (${((premiumHasEnhancedData / 15) * 100).toFixed(1)}%)`,
			);
			...
			premiumData.jobs.slice(0, 3).forEach((job: any, index: number) => {
				...
				console.log(
					`   🎯 Score: ${job.unifiedScore?.overall?.toFixed(2)} | Work: ${job.work_arrangement || "N/A"}`,
				);
	...
	test("Premium Executive Seeker Gets C-Level and Senior Roles", async ({
		request,
	}) => {
		...
		if (matchesResponse.status() === 200) {
			...
			const executiveJobs = matchesData.jobs.filter((job: any) => {
				...
				return title.match(
					/director|chief|vp|vice president|head|senior|executive|principal|partner/,
				);
			});
	...
	test("Premium Startup vs Enterprise Preference Works", async ({
		request,
	}) => {
		...
		if (matchesResponse.status() === 200) {
			...
			const hasStartups = companies.some(
				(company) => company && company.length < 20, // Rough proxy for smaller companies
			);
			expect(hasTechGiants || hasStartups).toBe(true); // Should have variety
		}
	});

	test("Premium Complex Career Path Gets Balanced Distribution", async ({
		request,
	}) => {
		...
		if (matchesResponse.status() === 200) {
			...
			const businessFunctions = matchesData.jobs.filter((job: any) => {
				...
				return titleAndDesc.match(
					/marketing|sales|finance|operations|hr|consulting|strategy|product|data/,
				);
			});
	...
	test("Premium Detailed Visa Requirements Met", async ({ request }) => {
		...
		if (matchesResponse.status() === 200) {
			...
			const highlyVisaFriendly = matchesData.jobs.filter(
				(job: any) =>
