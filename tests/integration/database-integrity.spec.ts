...
test.describe("Database Integrity", () => {
    ...
    test("Premium user data isolation and enhanced features", async ({ request }) => {
        ...
        if (response.status() === 200) {
            ...
            expect(data.premiumFeatures).toHaveProperty("enhancedVisaConfidence", true);
            expect(data.premiumFeatures).toHaveProperty("detailedJobData", true);
            expect(data.premiumFeatures).toHaveProperty("expandedTargetCompanies", true);
            ...
            if (data.jobs.length > 0) {
                ...
                if (firstJob.visa_confidence) {
                    expect(["verified", "likely", "local-only", "unknown"]).toContain(firstJob.visa_confidence);
                }
            }
    ...
    test("Data consistency across API calls", async ({ request }) => {
        ...
        if (createResponse.status() === 200) {
            ...
            if (matchesData.jobs.length > 1) {
                const cities = [...new Set(matchesData.jobs.map((job: any) => job.city))];
