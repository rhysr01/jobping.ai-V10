...
export class MatchingMetrics {
	...
	static recordMetrics(data: Omit<MatchingMetricsData, "timestamp">): void {
		...
		MatchingMetrics.metrics.push(metricsData);

		// Maintain bounded history
		if (MatchingMetrics.metrics.length > MatchingMetrics.MAX_METRICS_HISTORY) {
			MatchingMetrics.metrics = MatchingMetrics.metrics.slice(
				-MatchingMetrics.MAX_METRICS_HISTORY,
			);
		}

		// Log significant events
		MatchingMetrics.logSignificantMetrics(metricsData);
	}
	...
	static getTierStats(tier: "free" | "premium_pending"): PerformanceStats {
		const tierMetrics = MatchingMetrics.metrics.filter((m) => m.tier === tier);
		...
		const successfulRequests = tierMetrics.filter(
			(m) => m.matchCount > 0,
		).length;
		const aiRequests = tierMetrics.filter(
			(m) => m.aiRequests && m.aiRequests > 0,
		).length;
		const fallbackRequests = tierMetrics.filter((m) => m.fallbackUsed).length;

		return {
			averageDuration:
				tierMetrics.reduce((sum, m) => sum + m.duration, 0) / totalRequests,
			averageMatches:
				tierMetrics.reduce((sum, m) => sum + m.matchCount, 0) / totalRequests,
	...
	static getTierComparison(): TierComparison {
		const freeStats = MatchingMetrics.getTierStats("free");
		const premiumStats = MatchingMetrics.getTierStats("premium_pending");

		const ratio = {
			duration:
				premiumStats.averageDuration > 0
					? freeStats.averageDuration / premiumStats.averageDuration
					: 0,
			matches:
				premiumStats.averageMatches > 0
					? premiumStats.averageMatches / freeStats.averageMatches
					: 0,
			quality: MatchingMetrics.calculateQualityRatio(freeStats, premiumStats),
		};
	...
	private static calculateQualityRatio(
		...
		const freeQuality =
			free.successRate * 0.4 +
			free.aiUsageRate * 0.4 +
			(1 - free.fallbackRate) * 0.2;
		const premiumQuality =
			premium.successRate * 0.4 +
			premium.aiUsageRate * 0.4 +
			(1 - premium.fallbackRate) * 0.2;
	...
	static getRecentMetrics(hours: number = 24): MatchingMetricsData[] {
		...
		return MatchingMetrics.metrics.filter((m) => m.timestamp >= cutoff);
	}
	...
	static analyzeTrends(): {
		...
		const recent = MatchingMetrics.getRecentMetrics(24);
		const older = MatchingMetrics.getRecentMetrics(168); // Last 7 days
		...
		const recentAvgMatches =
			recent.reduce((sum, m) => sum + m.matchCount, 0) / recent.length;
		const olderAvgMatches =
			older.reduce((sum, m) => sum + m.matchCount, 0) / older.length;
		...
		// Additional insights
		const fallbackRate =
			recent.filter((m) => m.fallbackUsed).length / recent.length;
		if (fallbackRate > 0.1) {
			insights.push(`High fallback rate: ${(fallbackRate * 100).toFixed(1)}%`);
		}

		const lowConfidenceRate =
			recent.filter((m) => m.confidence === "low").length / recent.length;
		if (lowConfidenceRate > 0.2) {
			insights.push(
				`High low-confidence rate: ${(lowConfidenceRate * 100).toFixed(1)}%`,
			);
		}
	...
	static exportMetrics(): {
		...
		return {
			summary: MatchingMetrics.getTierComparison(),
			recentActivity: MatchingMetrics.getRecentMetrics(24),
			trends: MatchingMetrics.analyzeTrends(),
		};
	}
