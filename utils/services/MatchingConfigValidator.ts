...
export class MatchingConfigValidator {
	...
	static validateConfig(config: MatchingConfig): ValidationResult {
		...
		const { min, max } = MatchingConfigValidator.CONFIG_CONSTRAINTS.maxMatches;
		...
		const freshnessConstraints =
			MatchingConfigValidator.CONFIG_CONSTRAINTS.jobFreshnessDays;
		if (
			config.jobFreshnessDays < freshnessConstraints.min ||
			config.jobFreshnessDays > freshnessConstraints.max
		) {
			errors.push(
				`jobFreshnessDays must be between ${freshnessConstraints.min} and ${freshnessConstraints.max}`,
			);
		}
		...
		const aiConstraints =
			MatchingConfigValidator.CONFIG_CONSTRAINTS.maxJobsForAI;
		if (
			config.maxJobsForAI < aiConstraints.min ||
			config.maxJobsForAI > aiConstraints.max
		) {
			errors.push(
				`maxJobsForAI must be between ${aiConstraints.min} and ${aiConstraints.max}`,
			);
		}

		// Validate fallback threshold
		const fallbackConstraints =
			MatchingConfigValidator.CONFIG_CONSTRAINTS.fallbackThreshold;
		if (
			config.fallbackThreshold < fallbackConstraints.min ||
			config.fallbackThreshold > fallbackConstraints.max
		) {
			errors.push(
				`fallbackThreshold must be between ${fallbackConstraints.min} and ${fallbackConstraints.max}`,
			);
		}
	...
	static validateUserPreferences(
		...
		// Email validation
		if (!prefs.email || !MatchingConfigValidator.isValidEmail(prefs.email)) {
		...
		switch (prefs.subscription_tier) {
			case "free":
				MatchingConfigValidator.validateFreeTierPreferences(
					prefs,
					errors,
					warnings,
				);
			...
			case "premium_pending":
				MatchingConfigValidator.validatePremiumTierPreferences(
					prefs,
					errors,
					warnings,
				);
	...
	static validateCompleteSetup(
		...
		const configResult = MatchingConfigValidator.validateConfig(config);
		const prefsResult =
			MatchingConfigValidator.validateUserPreferences(preferences);
