...
export class ErrorRecoveryStrategies {
	...
	static async executeWithRecovery(
		...
		const config = { ...ErrorRecoveryStrategies.DEFAULT_OPTIONS, ...options };
		...
		for (let level = 1; level <= config.maxRecoveryLevel; level++) {
			try {
				const recoveryResult = await ErrorRecoveryStrategies.applyRecoveryLevel(
					level,
					userPrefs,
					jobs,
					config,
		...
		return await ErrorRecoveryStrategies.finalFallback(
			userPrefs,
			jobs,
			startTime,
		);
	...
	private static async applyRecoveryLevel(
		...
		switch (level) {
			case 1:
				return await ErrorRecoveryStrategies.level1RelaxedFiltering(
					userPrefs,
					jobs,
				);
			case 2:
				return await ErrorRecoveryStrategies.level2CityExpansion(
					userPrefs,
					jobs,
					config,
				);
	...
	private static async level1RelaxedFiltering(
		...
		const matches = jobs.filter((job) => {
			const cityMatch = ErrorRecoveryStrategies.matchesCityRelaxed(
				job,
		...
		const rankedMatches = ErrorRecoveryStrategies.rankByBasicRelevance(
			matches,
			userPrefs,
		);
	...
	private static async level2CityExpansion(
		...
		const expandedCities = ErrorRecoveryStrategies.expandCities(
			userPrefs.target_cities,
		);
	...
	private static async level4IndustryBroadening(
		...
		const broadenedIndustries = ErrorRecoveryStrategies.broadenIndustries(
			userPrefs.industries,
		);

		const matches = jobs.filter((job) => {
			const industryMatch = ErrorRecoveryStrategies.matchesIndustryBroadened(
				job,
				broadenedIndustries,
			);
		...
		const rankedMatches = ErrorRecoveryStrategies.rankByIndustryRelevance(
			matches,
			userPrefs,
		);
	...
	private static matchesCityRelaxed(
		job: JobWithMetadata,
		cities: string[],
	): boolean {
	...
	private static expandCities(cities: string[]): string[] {
		...
		cities.forEach((city) => {
			...
			if (expansions[cityLower]) {
				expansions[cityLower].forEach((expandedCity) =>
					expanded.add(expandedCity),
				);
			}
		});
	...
	private static rankByBasicRelevance(
		jobs: JobWithMetadata[],
		_userPrefs: UserPreferences,
	): JobWithMetadata[] {
		return jobs.sort((a, b) => {
			// Prefer jobs with more complete information
			const aScore = ErrorRecoveryStrategies.calculateCompletenessScore(a);
			const bScore = ErrorRecoveryStrategies.calculateCompletenessScore(b);
	...
	private static rankByIndustryRelevance(
		...
		if (!userPrefs.industries)
			return ErrorRecoveryStrategies.rankByBasicRelevance(jobs, userPrefs);

		return jobs.sort((a, b) => {
			const aIndustryScore =
				ErrorRecoveryStrategies.calculateIndustryMatchScore(
					a,
					userPrefs.industries!,
				);
			const bIndustryScore =
				ErrorRecoveryStrategies.calculateIndustryMatchScore(
					b,
					userPrefs.industries!,
				);
	...
	private static calculateIndustryMatchScore(
		job: JobWithMetadata,
		...
		const jobIndustries = job.ai_labels.map((label) => label.toLowerCase());
		const userIndustriesLower = userIndustries.map((industry) =>
