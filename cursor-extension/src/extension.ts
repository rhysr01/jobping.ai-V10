	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async () => {
				...
				const result = await sentryMCP.analyzeErrorPatterns(
		...
		vscode.commands.registerCommand("jobping.testMCPConnection", async () => {
			...
			if (hasAuthToken && hasOrg && hasProject) {
				vscode.window.showInformationMessage(
					"MCP Connection: Ready! Sentry tools should work.",
				);
			} else {
				vscode.window.showErrorMessage(
					"MCP Connection: Incomplete. Check your .env.local file.",
				);
			}
		}),
	);
