...
async function fixCategorization() {
	...
	try {
		...
		console.log(
			`   Internships: ${internships} (${((internships / jobs.length) * 100).toFixed(1)}%)`,
		);
		console.log(
			`   Graduates: ${graduates} (${((graduates / jobs.length) * 100).toFixed(1)}%)`,
		);
		console.log(
			`   Early Career: ${earlyCareer} (${((earlyCareer / jobs.length) * 100).toFixed(1)}%)`,
		);
