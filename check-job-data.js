...
async function checkJobData() {
	...
	try {
		...
		const { data: sampleJobs, error: sampleError } = await supabase
			.from("jobs")
			.select(
				"id, title, company, location, city, country, categories, experience_required",
			)
			.eq("is_active", true)
			.eq("status", "active")
			.limit(10);
		...
		if (catError) {
			console.log(
				"❌ Category distribution RPC not available, using fallback query...",
			);
		...
		if (londonError) {
			...
			londonJobs.forEach((job) => {
				console.log(
					`   ${job.title} @ ${job.company} - Categories: ${JSON.stringify(job.categories)}`,
				);
			});
		}
		...
		if (techError) {
			...
			techJobs.forEach((job) => {
				console.log(
					`   ${job.title} - Categories: ${JSON.stringify(job.categories)}`,
				);
			});
		}
	} catch (error) {
