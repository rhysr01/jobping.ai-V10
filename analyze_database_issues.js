...
async function analyzeDatabaseIssues() {
  ...
  console.log(`Active ratio: ${((activeJobs / totalJobs) * 100).toFixed(1)}%\n`);
  ...
  const { data: unpaidJobs, error: unpaidError } = await supabase
    ...
    .select('id', { count: 'exact', head: true })
    .eq('is_active', true)
    .or('description.ilike.%unpaid%,description.ilike.%internship only%,description.ilike.%no salary%');
  ...
  if (locationIssues > 0) {
    console.log(`Conflicting remote/on-site requirements: ${locationIssues} jobs`);
  }
